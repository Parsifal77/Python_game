...
class Hero(Gamesprite):
    def move(self):
        ...
        if keys[pygame.K_RIGHT] and self.rect.x < 350:
            self.rect.x += self.speed
...
def gaming(game, finish, e_number, y, y1, hp_image, w, exp_counter, hp_counter, kills):
    while game:
        ...
        if finish != True:
            ...
            if e_number == 0:
                e_number = 35
                enemy = Enemy('eee-min.png', randint(5, 350), -100, 100, 100, randint(2, 10))
                enemies.add(enemy)
            else:
                e_number -= 1
            enemies.draw(window)
            ...
            if kills >= 20:
                finish = True
                background_win = pygame.image.load('ggg (1).jpg')
                background_win = pygame.transform.scale(background, (500, 700))
                win = font1.render('You won', True, (0, 255, 0))
                window.blit(background_win, (0,0))
                window.blit(win, (180, 325))
            ...
            if hp_counter >= 11:
                finish = True
                background_over = pygame.image.load('ggg (1).jpg')
                background_over = pygame.transform.scale(background, (500, 700))
                game_over = font1.render('Game over', True, (255, 0, 0))
                window.blit(background_over, (0,0))
                window.blit(game_over, (180, 325))
                pygame.display.update()
                clock1.tick(60)
        pygame.display.update()
        clock.tick(60)
...
background1 = pygame.transform.scale(background, (500, 700))
...
while run:
    ...
    menu = font1.render('Press any key to start', True, (255, 255, 255))
